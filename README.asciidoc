...
-s::
    ...
    size etc.)
-c [x]::
    Clean the cache: trim the cache size to x*100% of its maximum by removing
    the oldest objects.
	The default value is 0.9 (90%)
-C::
...
CLCACHE_DISABLE_AUTOCLEAN::
	Setting this variable will disable invoking clcache clean automatically when
	cache get full. Warning: cache can grow over 100%, use "clcache -c" to 
	reduce cache size periodically (or before build).
CLCACHE_HARDLINK::
